...
class MediaR2Cache:
    def __init__(
        ...
        *,
        ...
        downloader: Callable[[str, str], DownloadedImage] | None = None,
    ) -> None:
        self.config = config
        self.s3 = s3_client or build_r2_client(config)
        self.downloader = downloader or (
            lambda source_url, platform: download_media_image(
                source_url,
                platform=platform,
            )
        )
    ...
    def cache_image(self, source_url: str, *, platform: str) -> CachedImage:
        platform = normalize_media_platform(platform)
        downloaded = self.downloader(source_url, platform)
        key = (
            f"{self.config.object_prefix}/{platform}/"
        ...
        if not reused:
            try:
                self.s3.put_object(
                    ...
                    Metadata={"sha256": downloaded.content_hash, "source": platform},
                )
...
class InstagramR2Cache(MediaR2Cache):
    """Compatibility wrapper for existing Instagram callers and backfills."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            s3_client=s3_client,
            downloader=(
                (lambda source_url, _platform: downloader(source_url))
    ...
    def cache_image(self, source_url: str) -> CachedImage:
        return super().cache_image(source_url, platform="instagram")
...
def download_media_image(
    ...
    *,
    ...
    for redirect_count in range(max_redirects + 1):
        validate_source_url(current_url, platform=platform)
        request = Request(
            current_url,
            headers={
        ...
        try:
            ...
            if exc.code in {301, 302, 303, 307, 308} and redirect_count < max_redirects:
                ...
                if not location:
                    raise MediaCacheError(f"{platform_label} image redirect omitted Location.") from exc
                current_url = urljoin(current_url, location)
                continue
            raise MediaCacheError(f"{platform_label} image request failed with HTTP {exc.code}.") from exc
        except (OSError, URLError) as exc:
            raise MediaCacheError(f"{platform_label} image request failed: {type(exc).__name__}") from exc
            ...
            if declared_length:
                try:
                    if int(declared_length) > max_bytes:
                        raise MediaCacheError(f"{platform_label} image exceeds the configured size limit.")
                except ValueError as exc:
                    raise MediaCacheError(f"{platform_label} image returned an invalid Content-Length.") from exc
        ...
        if extension is None or signature_check is None or not signature_check(body):
            raise MediaCacheError(f"{platform_label} media response is not a supported image.")
    ...
    raise MediaCacheError(f"{platform_label} image exceeded the redirect limit.")
...
def validate_source_url(value: str, *, platform: str = "instagram") -> None:
    platform = normalize_media_platform(platform)
    parsed = urlsplit(value)
    hostname = (parsed.hostname or "").lower().rstrip(".")
    if parsed.scheme != "https" or not hostname or parsed.username or parsed.password:
        raise MediaCacheError(f"{platform.capitalize()} image URL must use HTTPS without credentials.")
    if platform == "instagram":
        allowed = any(
    ...
    if not allowed:
        raise MediaCacheError(f"{platform.capitalize()} image host is not allowlisted.")
    try:
        addresses = {item[4][0] for item in socket.getaddrinfo(hostname, 443, type=socket.SOCK_STREAM)}
    except OSError as exc:
        raise MediaCacheError(f"{platform.capitalize()} image host could not be resolved.") from exc
    if not addresses or any(not ipaddress.ip_address(address).is_global for address in addresses):
        raise MediaCacheError(f"{platform.capitalize()} image host resolved to a non-public address.")
